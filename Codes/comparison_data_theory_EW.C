...
void comparison_data_theory_EW(){
  setTDRStyle();
  ...
  hData_2_new->Sumw2();
  TH1D *hNLO_2_new = new TH1D("Theory_new_2","Theory_new_2",arr_size-1,bins);
  
  TH1D *hData_3_new = new TH1D("Data_new_3","Data_new_3",arr_size-1,bins);
  hData_3_new->Sumw2();
  TH1D *hNLO_3_new = new TH1D("Theory_new_3","Theory_new_3",arr_size-1,bins);

  
  for (int i = 1; i <= arr_size-1  ; i++)
    {
      hData_2_new->SetBinContent(i,hData_2->GetBinContent(i));
      ...
      hData_2_new->SetBinError(i,err_2*hData_2->GetBinContent(i));  
      double np_factor_2 = hNP_2->GetBinContent(i);
      double ew_factor_2 = hEW_2->GetBinContent(i);
      hNLO_2_new->SetBinContent(i,hNLO_2->GetBinContent(i) * np_factor_2 * ew_factor_2); 
     
      hData_3_new->SetBinContent(i,hData_3->GetBinContent(i));
      ...
      hData_3_new->SetBinError(i,err_3*hData_3->GetBinContent(i));  
      double np_factor_3 = hNP_3->GetBinContent(i);
      hNLO_3_new->SetBinContent(i,hNLO_3->GetBinContent(i) * np_factor_3); 
  ...
  hNLO_2_new->SetMarkerStyle(20);
  hNLO_2_new->SetMarkerColor(kRed-3);
  hNLO_2_new->SetLineColor(kRed-3); 
  hNLO_2_new->SetLineWidth(2);  
  ...
  hNLO_3_new->SetMarkerStyle(20);
  hNLO_3_new->SetMarkerColor(kGreen+2);
  hNLO_3_new->SetLineColor(kGreen+2);
  hNLO_3_new->SetLineWidth(2);   
  ...
  hNLO_2_new->GetXaxis()->SetRangeUser(300.0,2000.0);
  hData_3_new->GetXaxis()->SetRangeUser(300.0,1680.0);//2000.0);
  hNLO_3_new->GetXaxis()->SetRangeUser(300.0,1680.0);//2000.0);*/
  hData_2_new->GetYaxis()->SetRangeUser(0.0000001,1000000.);
  hData_2_new->Draw("e");
  hNLO_2_new->Draw("histsame][");
  hData_3_new->Draw("esame");
  hNLO_3_new->Draw("histsame][");
  ...
  legend1->AddEntry(hNLO_2_new,"CT10 #otimes NP #otimes EW","l");
  legend1->AddEntry(hData_3_new,"n_{j} #geq 3  ");
  legend1->AddEntry(hNLO_3_new,"CT10 #otimes NP","l");
  legend1->Draw("sames");
  ...
  c->SaveAs("/home/anter/Desktop/Analysis_8_HT_2/Thesis_Plots/Gifs/Comparison_data_theory_EW.gif");
  c->SaveAs("/home/anter/Desktop/Analysis_8_HT_2/Thesis_Plots/Pdfs/Comparison_data_theory_EW.pdf"); 
