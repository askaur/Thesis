...
void comparison_data_MC_samples_2_Pow_EW(){
  setTDRStyle();

  TFile *fdata_2 = TFile::Open("/home/anter/Desktop/Analysis_8_HT_2/NLO/Final_Unfolding/Rootfile_NLO_HT_2_150/Data_Xsection_HT_2_150.root");
  ...
  TFile *fExp_Unc = TFile::Open("/home/anter/Desktop/Analysis_8_HT_2/Exp_Unc/RootFiles_HT_2_150/Exp_Unc_all_2_sources_NP.root");
  
  TFile *fExp_Unc_copy = TFile::Open("/home/anter/Desktop/Analysis_8_HT_2/Exp_Unc/RootFiles_HT_2_150/Exp_Unc_all_2_sources_NP_copy.root");
  
  TFile *fEW_2 = TFile::Open("/home/anter/Desktop/Analysis_8_HT_2/AN_Final_Plots/RootFiles/EW_2.root");
  
  //*****************************************************************
  
  TH1D *hData_2 = (TH1D*)fdata_2->Get("Data_2_new");
  TH1D *hMad  = (TH1D*)fmad->Get("Hadd_GenMCJetHT_2_150_2_nopileup_New");
  TH1D *hpythia  = (TH1D*)fpythia->Get("jet/GenMCJetHt_2_2");
  TH1D *hherwig  = (TH1D*)fherwig->Get("jet/GenMCJetHt_2_2");
  TH1D *hpowheg_S1  = (TH1D*)fpowheg->Get("GenMCJetHt_2_2_S1");
  TH1D *hpowheg_M1  = (TH1D*)fpowheg->Get("GenMCJetHt_2_2_M1");  
  ...
  TH1D *hData_MC_ratio = new TH1D("Data_MC_ratio_2","Data_MC_ratio_2",arr_size-1,bins);
  hData_MC_ratio->Sumw2();
  
  TH1D *hData_MC_ratio_err = new TH1D("Data_MC_ratio_2_err","Data_MC_ratio_2_err",arr_size-1,bins);
  hData_MC_ratio_err->Sumw2();

  TH1D *hData_MC_ratio_unc = new TH1D("Data_MC_ratio_2_unc","Data_MC_ratio_2_unc",arr_size-1,bins);
  hData_MC_ratio_unc->Sumw2();
  
  TH1D *hherwig_new = new TH1D("herwig_new","herwig_new",arr_size-1,bins);
     
  hMad->Scale(1.0, "width");   
  
  for (int i = 1; i <= arr_size-1  ; i++)
    {
     if (hpowheg_S1->GetBinContent(i) == 0.0 )
     double data_bin_ratio = 1.;
     else
     double data_bin_ratio = hData_2->GetBinContent(i)/(hpowheg_S1->GetBinContent(i)*hEW_2->GetBinContent(i));
  ...
  hpythia->Scale(29981599700.0/181279898.0);
  hpythia->Scale(1.0, "width");
  
 // hherwig->Scale(847881020.0/180709224.0);    
  hherwig->Scale(23100000300.0/180709224.0);
  //hherwig->Scale((847881020.0/9934463.0)*7716810.);
  hherwig->Scale(1.0, "width");   
   
  //hData_2->Divide(hpowheg_S1);
  hpythia->Divide(hpowheg_S1);
  hherwig->Divide(hpowheg_S1);
  hMad->Divide(hpowheg_S1);
  hpowheg_M1->Divide(hpowheg_S1);  
  ...
  for (int i = 1; i <= arr_size-1  ; i++)
    {
    if (i == 33)
    hherwig_new->SetBinContent(i,0.66);
    else
     hherwig_new->SetBinContent(i,hherwig->GetBinContent(i));
    }
  ...
  hpythia->SetMarkerColor(kGreen+2);
  hpythia->SetLineColor(kGreen+2);
  hpythia->SetLineWidth(2);
  hpythia->SetLineStyle(9);       
  
  hMad->SetMarkerColor(38);
  hMad->SetLineColor(38);
  hMad->SetLineWidth(2);    
  
  hpowheg_M1->SetMarkerColor(kBlue);
  hpowheg_M1->SetLineColor(kBlue);
  hpowheg_M1->SetLineWidth(2);    
  hpowheg_M1->SetLineStyle(2);    
  ...
  hData_MC_ratio->GetXaxis()->SetRangeUser(300.0,2000.0);
  hData_MC_ratio_unc->GetXaxis()->SetRangeUser(300.0,2000.0);
  hpythia->GetXaxis()->SetRangeUser(300.0,2000.0);
  hherwig_new->GetXaxis()->SetRangeUser(300.0,2000.0);
  hExp_Unc_Up->GetXaxis()->SetRangeUser(300.0,2000.0);
  hpowheg_M1->GetXaxis()->SetRangeUser(300.0,2000.0);
  hMad->GetXaxis()->SetRangeUser(300.0,2000.0);  
  ...
  hpythia->Draw("histsame][");
  //hherwig_new->Draw("histsame][");
  hpowheg_M1->Draw("histsame][");
  hMad->Draw("histsame]["); 
  ...
  legend1->AddEntry(hData_MC_ratio,"Data/EW");
  legend1->AddEntry(hpythia,"Pythia6 Z2*","l");
  legend1->AddEntry(hMad,"MG+P6 Z2*","l");
  legend1->AddEntry(hpowheg_M1,"Powheg+Pythia8 CUETM1","l");  
  ...
  c->SaveAs("/home/anter/Desktop/Analysis_8_HT_2/Thesis_Plots/Gifs/Comparison_data_MC_samples_2_Pow_EW.gif");
  c->SaveAs("/home/anter/Desktop/Analysis_8_HT_2/Thesis_Plots/Pdfs/Comparison_data_MC_samples_2_Pow_EW.pdf"); 
}  
